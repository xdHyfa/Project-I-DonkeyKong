...
void Level2FireRoutine(Fire& fire) {
    ...
    if (fire.has_Spawned) {
        ...
        // Use blue texture during hammer time
        Texture2D& tex = GetHammerTime() ? fire.BlueTexture : fire.Texture;

        fire.bounceTick += GetFrameTime();
        if (fire.bounceTick >= 0.5f) {
            DrawTextureRec(tex, fire.FireSprite, fire.Position, WHITE);
            ...
            DrawTextureRec(tex, fire.FireSprite, BouncePos, WHITE);
        }
...
void UnloadLevel2Entities() {
    ...
    UnloadTexture(Fire3.BlueTexture);
    UnloadTexture(Fire4.Texture);
    UnloadTexture(Fire4.BlueTexture);
    UnloadTexture(Fire5.Texture);
    UnloadTexture(Fire5.BlueTexture);
    UnloadTexture(Fire6.Texture);
    UnloadTexture(Fire6.BlueTexture);
    UnloadTexture(Fire7.Texture);
    UnloadTexture(Fire7.BlueTexture);
}
